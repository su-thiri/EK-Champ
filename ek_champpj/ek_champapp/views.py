...
def base_round(request):
    data = Round.objects.raw('select * from ')
    return render(request,'pages/round.html',{'data':data})
...
def store_round(request):
    ...
    rdata.pitlane = request.POST.get('pitlan')
    rdata.save
...
def editdata_round(request,pk):
    ...
    edata.pitlane = request.POST.get('pitlan')
    edata.save()
    return redirect('/base_round')

def base_teamno(request):
    data = Team_numbers.objects.raw('select * from ')
    return render(request,'teamno.html',{'data':data})

def create_teamno(request):
    return render(request,'create_teammo.html')
...
def base_tinround(request):
    data = Team_in_round.objects.raw('select * from ')
    return render(request,'tinround.html',{'data':data})

def crete_tinround(request):
